...
class Oring:
    ...
    def __post_init__(self):
        ...
        if self.oring_usage not in (
            ...
            raise ValueError(
                'usage must be "static", "dynamic", "internal_pressure", or "external_pressure"'
            )
        ...
        if self.oring_type in ("rod_seal", "piston_seal") and self.oring_usage in (
            "internal_pressure",
            "external_pressure",
        ):
...
class OringCalc:
    ...
    def evaluate(
        self, oring: Oring, groove: FaceGroove | RodGroove | PistonGroove
    ) -> None | bool:
        condition_couples = [
            ("face_seal", FaceGroove),
            ("rod_seal", RodGroove),
            ("piston_seal", PistonGroove),
        ]
        ...
        for oring_type, groove_type in condition_couples:
            if oring.oring_type == oring_type and not isinstance(groove, groove_type):
                logger.critical(
                    f"{oring_type} orings are only compatible for {oring_type} grooves"
                )
                return None
        ...
        if oring.oring_type == "face_seal":
            ...
            logger.critical(
                "This part is under constuction, no Rod or Piston seal calculation is available"
            )
            return None

    def _evaluate_face_seal(self, oring: Oring, groove: FaceGroove) -> None | bool:
        groove_width = (groove.groove_od - groove.groove_id) / 2

        compression_ratio = calc_helper.get_compression_ratio(
            CS=oring.cross_section_dia, W=groove.groove_depth
        )
        logger.debug(f"oring compression ratio (%): {compression_ratio:.1f}")

    def suggest_groove(
        self, oring: Oring, groove_boundaries: FaceGroove | RodGroove | PistonGroove
